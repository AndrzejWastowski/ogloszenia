...
<div class="row justify-content-center">
    ...
    <h3><strong>Motoryzacja - Dodaj Treść</strong></h3>
        ...
        @endif
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    ...
                    <input type="radio" class="form-check-input" id="cars_type" name="cars_type" value="Sprzedam" checked>
                </div>
                
                <div class="col-md-2">
                    <label class="form-label" for="oddam">Wynajem</label>
                    <input type="radio" class="form-check-input"  id="cars_type" name="cars_type" value="Wynajmę" >
                </div>
            </div>
            

            <div class="col-md-6">
                <label class="form-label" for="cars_brands_id"><strong>Kategoria</strong></label>
                <select class="form-select" name="cars_brands_id" id="cars_brands_id" required>
                    <option value="" diabled selected="">Wybierz kategorię</option>  
                        @foreach($brands as $category)
                            <option value="{{$category->id}}" {{ ($category->id == $content->cars_brands_id ? 'selected' : '') }} >{{$category->name}}</option>
                        @endforeach                            
                    </select>
            </div>
...
$( document ).ready(function() {
    ...
    $(document).on('change', '#cars_brands_id', function (e) {    
        ...
        $.get('/internal-api/add/models/' + cat_id,function(data){
